package Client.Fisc.command;

import java.util.HashMap;
import java.util.Map;

import Client.Fisc.Fisc;
import Client.Fisc.command.commands.BindCommand;
import Client.Fisc.command.commands.HelpCommand;
	...
	public boolean processCommand(String rawMessage) {
		...
		if (safe) {
			...
			if (command != null) {
				...
				if (!command.run(args)) {
					Fisc.INSTANCE.tellPlayer(command.usage());
				}
			}
			/** If no command was found it tell the user to do the help command. **/
			else {
				Fisc.INSTANCE.tellPlayer("Try -help.");
			}
		}
		/**
